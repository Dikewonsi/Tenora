      ...
      {modalOpen && <div className="tenora-property-modal-backdrop">
        <form className="tenora-property-modal tenora-budget-modal" onSubmit={handleSubmit}>
          ...
          <div className="tenora-property-modal-body">
            ...
            <div className="tenora-form-section"><div><strong>Calculation</strong><span>Choose how the budget is shared. Vacant and inactive unit shares remain owner liability.</span></div><div className="row g-3"><div className="col-12 col-md-6"><label className="form-label">Total budget</label><input className="form-control" type="number" min="0.01" step="0.01" value={form.total_budget} onChange={(event) => setForm((current) => ({ ...current, total_budget: event.target.value }))} required /></div><div className="col-12 col-md-6"><label className="form-label">Calculation method</label><select className="form-select" value={form.calculation_method} onChange={(event) => setForm((current) => ({ ...current, calculation_method: event.target.value }))}><option value="pro_rata">Pro rata by floor area</option><option value="flat_rate">Flat rate per physical unit</option></select><div className="form-text">{form.calculation_method === 'flat_rate' ? 'Each physical unit receives an equal share; only occupied active units get tenant demands.' : 'Each unit share is based on floor area divided by total lettable space.'}</div></div></div></div>
